...
<%
	...
	// 요청이 들어온 페이지 1은 첫번째 페이지 그 다음페이지는 1씩 증가
	int currentPage = 1;
	// 한 페이지에 보여줄 게시물 개수
	int pageCount = 5;
...
<%
	...
	String sql = "SELECT count(*) FROM test_guestbook ORDER BY id DESC";
	...
	// [6][7][8][9][10]  6~10페이지는 동일한 번호 출력
	// 시작페이지 번호
	// 1page : (1-1) / 5 * 5 + 1 == 1;
	// 5page : (5-1) / 5 == 0.8 == 0 * 5 == 0 + 1 == 1;
	// 6page : (6-1) / 5 == 1.0 == 1 * 5 == 5 + 1 == 6;
	...
	// endPage가 maxPage를 벗어나면 endPage 를 maxPage로 변경
	if(endPage > maxPage){
		endPage = maxPage;
	}
